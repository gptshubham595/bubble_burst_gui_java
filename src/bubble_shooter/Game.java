...
public class Game implements ActionListener {
	...
	/**
	 * number of the unsuccessful (no bubbles disappeared) shots. used for adding a
	 * top row when it reaches 5
	 */
	...
	/**
	 * true if the game is stopped (the highscore is being displayed), else false
	 */
	...
	/**
	 * the score received for removing a bubble of the same color as the one shot
	 */
	public static final int SCORE_COHERENT = 20;

	/**
	 * the score received for removing a bubble that anyway would just float on the
	 * field
	 */
	public static final int SCORE_FLOATING = 40;

	/**
	 * contructor for a new game. initalises the bubble matrix, the upcoming bubbles
	...
	public Game(int row, int colors, Canvas c) {
		...
		for (int i = 0; i < ROW_COUNT; i++) {
			...
			for (int j = 0; j < (r.isFull() ? 14 : 13); j++) {
				...
				b.setLocation(new Point(
						r.isFull() ? j * 2 * (Bubble.RADIUS + 1) : j * 2 * (Bubble.RADIUS + 1) + (Bubble.RADIUS + 1),
						r.isFull() ? (i / 2) * Constants.ROW_DISTANCE
	...
	/**
	 * shifts the upcoming bubbles by one. the first is removed and one is added to
	 * the end
	 */
	private void arrangeUpcoming() {
		...
		for (int i = 1; i < 4; i++) {
			upcoming.get(i).setLocation(new Point(Constants.FIELD_SIZE_X - (4 - i) * (2 * (Bubble.RADIUS + 6)),
	...
	/**
	 * checks whether the moving bubble is close to a fixed one. if yes, then it
	 * will be fixed in the hexile grid
	 */
	...
	private void addRow() {
		...
		for (int i = 0; i < (newRow.isFull() ? 14 : 13); i++) {
			Bubble b = new Bubble(Bubble.getRandomColor(colors));
			b.setLocation(new Point(
					(newRow.isFull() ? i * 2 * (Bubble.RADIUS + 1) : i * 2 * (Bubble.RADIUS + 1) + (Bubble.RADIUS + 1)),
					0));
			b.setVisible(true);
			newRow.add(b);
			numOfBubbles++;
	...
	/**
	 * removes all bubbles that would anyway just float in the grid (so not
	 * connected to the top of the field by a chain of bubbles)
	 * 
	 * @return the number of removed bubbles
	 */
	...
	/**
	 * unmarks not floating elements outgoing from the bubble gived with the
	 * coordinates
	 * 
	...
	/**
	 * marks the bubbles that have the same color as the one given with the
	 * coordinates (these ones must be somehow connected to the given one with a
	 * series of bubbles of the same color)
	 * 
	 * @param row row-index of the bubble
